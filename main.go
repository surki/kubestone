...
func main() {
	...
	if err = (&osbench.Reconciler{
		K8S: k8sAccess,
		Log: ctrl.Log.WithName("controllers").WithName("Osbench"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "Osbench")
		os.Exit(1)
	}
	if err = (&nighthawk.Reconciler{
		K8S: k8sAccess,
